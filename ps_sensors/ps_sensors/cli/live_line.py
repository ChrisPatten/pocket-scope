...
def format_sample_line(sampler: PpsSampler) -> str:
    ...
    fix_str = f"{gps.fix_type:5s}"
    age_str = f"{gps.age_s:4.1f}s"

    # IMU section
    acc_str = f"{imu.ax_g:7.3f} {imu.ay_g:7.3f} {imu.az_g:7.3f}"
    gyr_str = f"{imu.gx_dps:7.2f} {imu.gy_dps:7.2f} {imu.gz_dps:7.2f}"

    # Mag section
    if mag:
        mag_str = f"{mag.mx_uT:7.2f} {mag.my_uT:7.2f} {mag.mz_uT:7.2f}"
        dor_str = f"{mag.dor_total:4d}"
    else:
        mag_str = "    n/a     n/a     n/a"
        dor_str = " n/a"

    tick_str = f"{sample.tick.tick_count:5d}"
...
def live_display_curses(stdscr: "curses._CursesWindow", sampler: PpsSampler) -> None:
    ...
    stdscr.addstr(
        1,
        0,
        " Tick | Time     Lat       Lon       Fix   Age | Accel X   Y   Z (g)     | Gyro X   Y   Z (°/s)    | Mag X   Y   Z (µT)      | DOR ",
    )
    stdscr.addstr(
        2,
        0,
