...
class MapDataProvider:
    ...
    def __init__(
        self, db_path: str, cache: cache_module.LRUCache | None = None
    ) -> None:
        self.db_path = db_path
        self.cache = cache or cache_module.LRUCache()

    def get_features_near(
        self, lat: float, lon: float, extra_airports: list[str] | None = None
    ) -> Dict[str, Any]:
        ...
        with get_connection(self.db_path) as conn:
            airports = query_airports(
                conn, lat, lon, minx, miny, maxx, maxy, RADIUS_MI, extra_airports
            )
