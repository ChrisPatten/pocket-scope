...
def _merge_theme_data(
    ...
    base_themes = merged.get("themes")
    if isinstance(base_themes, Mapping):
        themes: Dict[str, Any] = {
            str(k): dict(v) if isinstance(v, Mapping) else v
            for k, v in base_themes.items()
        }
    else:
        themes = {}
    new_themes = override.get("themes") if isinstance(override, Mapping) else None
    if isinstance(new_themes, Mapping):
